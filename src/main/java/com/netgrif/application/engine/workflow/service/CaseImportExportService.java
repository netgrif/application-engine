...
import com.netgrif.application.engine.utils.archive.interfaces.IArchiveService;
...
public class CaseImportExportService implements ICaseImportExportService {
    ...
    public List<Case> importCases(InputStream inputStream) {
        ...
        return new ArrayList<>(saveImportedObjects(importedCases, importer.getImportedTasksMap()).values());
    }
    ...
    private void saveFiles(List<String> casesDirectories, Map<String, Case> importedIdsMapping) throws IOException, StorageException {
        for (String caseDirectory : casesDirectories) {
            ...
            List<String> fieldsOfCaseDirectories = Arrays.stream(Objects.requireNonNull(new File(caseDirectory).list(DirectoryFileFilter.DIRECTORY))).collect(Collectors.toList());

...
public class PdfEnumerationMapFieldBuilder extends PdfFormFieldBuilder<PdfEnumerationMapField> {
    ...
    public String[] getType() {
        return new String[]{DataType.ENUMERATION_MAP.value()};
    }
    ...
    protected void setupValue(PdfBuildingBlock buildingBlock, PdfEnumerationMapField pdfField) {
        ...
        if (field.getValue() != null && field.getValue().getValue() != null) {
            Set<String> values = new HashSet<>(generateMultiLineText(Collections.singletonList(field.getOptions().get(field.getValue().getValue()).getTranslation(buildingBlock.getLocale())), maxValueLineLength));

...
public class PdfMultiChoiceMapFieldBuilder extends PdfFormFieldBuilder<PdfMultiChoiceMapField> {
    ...
    public PdfMultiChoiceMapField buildField(PdfBuildingBlock buildingBlock) {
        ...
        PdfMultiChoiceMapField pdfField = new PdfMultiChoiceMapField(((PdfFormFieldBuildingBlock) buildingBlock).getDataRef().getField().getStringId());
    ...
    public String[] getType() {
        return new String[]{DataType.MULTICHOICE_MAP.value()};
    }
    ...
    protected void setupValue(PdfBuildingBlock buildingBlock, PdfMultiChoiceMapField pdfField) {
        ...
        if (field.getValue() != null && field.getValue().getValue() != null) {
            Set<String> values = new HashSet<>(generateMultiLineText(Collections.singletonList(field.getValue().getValue().stream().map(v -> field.getOptions().get(v).getTranslation(buildingBlock.getLocale())).collect(Collectors.joining(", "))), maxValueLineLength));

...
public class PetriNetController {
    ...
    public EntityModel<EventOutcomeWithMessage> importPetriNet(
        ...
        try {
            ...
            ImportPetriNetEventOutcome importPetriNetOutcome = service.importPetriNet(multipartFile.getInputStream(), release, (Identity) auth.getPrincipal(), uriNodeId);
    ...
    public PetriNetReferenceResources getAll(@RequestParam(value = "identifier", required = false) String identifier, @RequestParam(value = "version", required = false) String version, Authentication auth, Locale locale) {
        Identity user = (Identity) auth.getPrincipal();
    ...
    public PetriNetReferenceResource getOne(@PathVariable("identifier") String identifier, @PathVariable("version") String version, Authentication auth, Locale locale) {
        ...
        return new PetriNetReferenceResource(service.getReference(resolvedIdentifier, converter.convert(version), (Identity) auth.getPrincipal(), locale));
    }
    ...
    public TransitionReferencesResource getTransitionReferences(@RequestParam List<String> ids, Authentication auth, Locale locale) {
        ...
        return new TransitionReferencesResource(service.getTransitionReferences(ids, (Identity) auth.getPrincipal(), locale));
    }

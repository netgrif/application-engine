...
public class MailService implements IMailService {
    ...
    public void sendRegistrationEmail(RegisteredUser user) throws MessagingException, IOException, TemplateException {
        ...
        MailDraft mailDraft = MailDraft.builder(mailProperties.getFrom(), recipients).subject(EmailType.REGISTRATION.getSubject())
    ...
    public void sendPasswordResetEmail(RegisteredUser user) throws MessagingException, IOException, TemplateException {
        ...
        MailDraft mailDraft = MailDraft.builder(mailProperties.getFrom(), Collections.singletonList(user.getEmail())).subject(EmailType.PASSWORD_RESET.getSubject())
...
//        }
    ...
    protected String getServerURL() {
        String encryptedHttp = mailProperties.getRedirectTo().isSsl() ? "https://" : "http://";
        String usedPort = mailProperties.getRedirectTo().getPort() != null && !mailProperties.getRedirectTo().getPort().isEmpty() ? (":" + mailProperties.getRedirectTo().getPort()) : "";
        return encryptedHttp + mailProperties.getRedirectTo().getHost() + usedPort;
    }
}
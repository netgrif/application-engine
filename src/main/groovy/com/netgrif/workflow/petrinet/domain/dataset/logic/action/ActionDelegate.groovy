...
class ActionDelegate {
    ...
    IUser assignRole(String roleMongoId, IUser user = userService.loggedUser) { // userDetailsService.reloadSecurityContext(userService.getLoggedUser().transformToLoggedUser())
    ...
    Set<Group> findOrganisation(IUser user = loggedUser()) {
        return memberService.findByEmail(user.email)?.groups
    }

    Group createOrganisation(String name, Group parent = null, Set<IUser> users = [] as Set) {
    ...
    Group removeMember(Group organisation, IUser user) {
        organisation.members.removeAll { it.email == user.email }
        return groupService.save(organisation)
    }

    Group addMember(Group organisation, IUser user) {
        def member = findMember(user)
        organisation.members.add(member)
        return groupService.save(organisation)
    }

    Member findMember(IUser user) {

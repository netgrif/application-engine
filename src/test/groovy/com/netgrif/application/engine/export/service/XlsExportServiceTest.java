...
public class XlsExportServiceTest {
    ...
    void shouldCreateXlsxFile() throws Exception {
        LoggedUser user = getSuperUser();

        IntStream.range(0,5).forEach(idx -> workflowService.createCaseByIdentifier("preference_item", "Test case", "", user));

        FilteredCasesRequest request = getTestRequest();
        File excel = xlsExportService.getExportFilteredCasesFile(request, getSuperUser(), Locale.ENGLISH);
        assertNotNull(excel);
    ...
    LoggedUser getSuperUser() {
        IUser user = userService.findByEmail("super@netgrif.com", true);
    ...
    FilteredCasesRequest getTestRequest() {
        ...
        request.setQuery(List.of(
                CaseSearchRequest.builder()
                        .query("processIdentifier:preference_item")
                        .build()));

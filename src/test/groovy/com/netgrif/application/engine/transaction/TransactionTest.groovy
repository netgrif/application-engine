...
class TransactionTest {
    ...
    void before() throws IOException, MissingPetriNetMetaDataException {
        ...
        testProcess = petriNetService.importProcess(new ImportProcessParams(
    ...
    void testBasicTransactionWithFailure() {
        importHelper.createCase("toBeRemoved", testProcess)
        ...
        assert useCase.getDataSet().get("text_without_action").getValue() == null
    ...
    void testElasticIndexingOnTransactionFailure() {
        ...
        assert countTestCaseTasksElastic() == 3
    ...
    void testElasticIndexingOnTransactionSuccess() {
        ...
        Thread.sleep(4000) // wait for indexation
        ...
        assert countTestCaseTasksElastic() == 5
    ...
    void testPerformance() {
        ...
        (0..iterations).each {idx ->
            Pair<TimeDuration, Case> createCasePair = createCase(process.stringId, "Case" + idx)
    ...
    private long countTestCaseTasks() {
        return taskRepository.count(QTask.task.processId.eq(testProcess.getStringId()) & QTask.task.transitionId.eq("t1"));
    }
    ...
    private long countTestCaseTasksElastic() {
        return elasticTaskService.count(
                List.of(new ElasticTaskSearchRequest(Map.of("query", (Object) ("processId:" + testProcess.getStringId() + " AND transitionId:t1")))),
                superCreator.getLoggedSuper().activeActorId,
                Locale.getDefault(),
                false
        )
    ...
    private Pair<TimeDuration, Case> createCase(String petriNetStringId, String caseTitle) {
        ...
        return Pair.of(TimeCategory.minus( endTime, startTime ), ((CreateCaseEventOutcome)((EventOutcomeWithMessage) parseResult(result)).outcome).case)
    }
    ...
    private Pair<TimeDuration, Task> assignTask(String taskId) {
        ...
        MvcResult result = mvc.perform(get(ASSIGN_TASK_URL+taskId)
                ...
                .andReturn()
        Date endTime = new Date()
        return Pair.of(TimeCategory.minus( endTime, startTime ), ((AssignTaskEventOutcome)((EventOutcomeWithMessage) parseResult(result)).outcome).task)
    }
    ...
    private Pair<TimeDuration, Task> finishTask(String taskId) {
        ...
        MvcResult result = mvc.perform(get(FINISH_TASK_URL+taskId)
                ...
                .andReturn()
        Date endTime = new Date()
        return Pair.of(TimeCategory.minus( endTime, startTime ), ((FinishTaskEventOutcome)((EventOutcomeWithMessage) parseResult(result)).outcome).task)
    }

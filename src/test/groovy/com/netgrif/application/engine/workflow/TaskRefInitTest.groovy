...
class TaskRefInitTest {
    ...
    void initNet() {
        ...
        net = petriNetService.importPetriNet(new ImportPetriNetParams(
                new FileInputStream("src/test/resources/taskref_init.xml"), VersionType.MAJOR, userService.loggedOrSystem.transformToLoggedUser())).getNet()
        autoTrigger = petriNetService.importPetriNet(new ImportPetriNetParams(
                new FileInputStream("src/test/resources/autotrigger_taskref.xml"), VersionType.MAJOR, userService.loggedOrSystem.transformToLoggedUser())).getNet()
        assert net != null
    }

...
public class ActorTransformer {
    ...
    public static ActorRef toActorRef(AbstractUser user) {
        return new ActorRef(
                ...
                user.getUsername(),
                user.getName(),
                user.getAuthoritySet().stream().map(Authority::getName).collect(Collectors.toSet())
        );
    }
    ...
    public static ActorRef toActorRef(LoggedUser loggedUser) {
        return new ActorRef(
                ...
                loggedUser.getName(),
                loggedUser.getAuthoritySet().stream().map(Authority::getName).collect(Collectors.toSet())
        );
    }
    ...
    public static ActorRef toActorRef(Group group) {
        return new ActorRef(
                ...
                group.getIdentifier(),
                group.getName(),
                group.getAuthoritySet().stream().map(Authority::getName).collect(Collectors.toSet())
        );
    }

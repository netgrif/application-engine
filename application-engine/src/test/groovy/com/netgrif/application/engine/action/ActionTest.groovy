...
class ActionTest {
    ...
    void testActionImports() {
        Case dummy = new com.netgrif.application.engine.adapter.spring.workflow.domain.Case()
        Task task = com.netgrif.application.engine.adapter.spring.workflow.domain.Task.with()._id(new ProcessResourceId()).transitionId("0").build()
        Action testAction = new Action('''
            println LocalDate.now()
            println LocalDate.MAX
            println new com.netgrif.application.engine.objects.workflow.domain.ProcessResourceId().toString()
        ''', "set")

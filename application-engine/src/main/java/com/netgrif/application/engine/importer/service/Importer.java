...
public class Importer {
    ...
    protected List<com.netgrif.application.engine.objects.petrinet.domain.Function> functions;
    ...
    protected void applyMapping(Mapping mapping) throws MissingIconKeyException {
        ...
        for (com.netgrif.application.engine.objects.importer.model.DataGroup dataGroup : mapping.getDataGroup()) {
    ...
    protected void resolveDataActions(Data data) {
        ...
        if (data.getAction() != null) {
            ...
            List<com.netgrif.application.engine.objects.importer.model.Action> filteredActions = filterActionsByTrigger(data.getAction(), DataEventType.GET);
    ...
    private List<com.netgrif.application.engine.objects.importer.model.Action> filterActionsByTrigger(List<com.netgrif.application.engine.objects.importer.model.Action> actions, DataEventType trigger) {
...
//      It has to be here for backwards compatibility of variable arcs
    ...
    protected void createTransition(com.netgrif.application.engine.objects.importer.model.Transition importTransition) throws MissingIconKeyException {
        ...
        if (importTransition.getDataRef() != null) {
            for (com.netgrif.application.engine.objects.importer.model.DataRef dataRef : importTransition.getDataRef()) {
        ...
        if (importTransition.getDataGroup() != null) {
            for (com.netgrif.application.engine.objects.importer.model.DataGroup dataGroup : importTransition.getDataGroup()) {
    ...
    protected com.netgrif.application.engine.objects.petrinet.domain.events.ProcessEvent addProcessEvent(com.netgrif.application.engine.objects.importer.model.ProcessEvent imported) {
        ...
        event.setMessage(toI18NString(imported.getMessage()));
        event.setImportId(imported.getId());
        event.setType(com.netgrif.application.engine.objects.petrinet.domain.events.ProcessEventType.valueOf(imported.getType().value().toUpperCase()));
    ...
    protected List<Action> parsePostActions(String transitionId, com.netgrif.application.engine.objects.importer.model.BaseEvent imported) {
        return parsePhaseActions(EventPhaseType.POST, transitionId, imported);
    }

    protected List<Action> parsePreActions(String transitionId, com.netgrif.application.engine.objects.importer.model.BaseEvent imported) {
        return parsePhaseActions(EventPhaseType.PRE, transitionId, imported);
    }

    protected List<Action> parsePhaseActions(EventPhaseType phase, String transitionId, com.netgrif.application.engine.objects.importer.model.BaseEvent imported) {
    ...
    protected List<Action> parsePhaseActions(String fieldId, EventPhaseType phase, DataEventType trigger, String transitionId, com.netgrif.application.engine.objects.importer.model.DataEvent dataEvent) {
    ...
    protected Map<DataEventType, DataEvent> buildEvents(String fieldId, List<com.netgrif.application.engine.objects.importer.model.DataEvent> events, String transitionId) {
        ...
        List<com.netgrif.application.engine.objects.importer.model.DataEvent> filteredEvents = events.stream()
    ...
    protected com.netgrif.application.engine.objects.petrinet.domain.events.DataEvent parseDataEvent(String fieldId, List<com.netgrif.application.engine.objects.importer.model.DataEvent> events, String transitionId) {
        com.netgrif.application.engine.objects.petrinet.domain.events.DataEvent dataEvent = new com.netgrif.application.engine.objects.petrinet.domain.events.DataEvent();
    ...
    protected com.netgrif.application.engine.objects.petrinet.domain.events.DataEvent createDataEvent(Action action) {
        com.netgrif.application.engine.objects.petrinet.domain.events.DataEvent dataEvent;
        if (action.getId() != null) {
            dataEvent = new com.netgrif.application.engine.objects.petrinet.domain.events.DataEvent(action.getId().toString(), action.getTrigger().toString());
        } else {
            dataEvent = new com.netgrif.application.engine.objects.petrinet.domain.events.DataEvent(new ObjectId().toString(), action.getTrigger().toString());
        }
        return dataEvent;
    }
    ...
    protected Action parseAction(String transitionId, com.netgrif.application.engine.objects.importer.model.Action action) {
    ...
    protected Action parseAction(String fieldId, String transitionId, com.netgrif.application.engine.objects.importer.model.Action importedAction) {
    ...
    protected Action createAction(com.netgrif.application.engine.objects.importer.model.Action importedAction) {
    ...
    protected void parseIds(String fieldId, String transitionId, com.netgrif.application.engine.objects.importer.model.Action importedAction, Action action) {
    ...
    protected Map<EventType, com.netgrif.application.engine.objects.petrinet.domain.events.Event> createEventsMap(List<com.netgrif.application.engine.objects.importer.model.Event> events) {
        Map<EventType, com.netgrif.application.engine.objects.petrinet.domain.events.Event> finalEvents = new HashMap<>();
        events.forEach(event ->
    ...
    protected Map<ProcessEventType, com.netgrif.application.engine.objects.petrinet.domain.events.ProcessEvent> createProcessEventsMap(List<com.netgrif.application.engine.objects.importer.model.ProcessEvent> events) {
        Map<ProcessEventType, com.netgrif.application.engine.objects.petrinet.domain.events.ProcessEvent> finalEvents = new HashMap<>();
        events.forEach(event ->
    ...
    protected Map<CaseEventType, com.netgrif.application.engine.objects.petrinet.domain.events.CaseEvent> createCaseEventsMap(List<com.netgrif.application.engine.objects.importer.model.CaseEvent> events) {
        Map<CaseEventType, com.netgrif.application.engine.objects.petrinet.domain.events.CaseEvent> finalEvents = new HashMap<>();
        events.forEach(event ->
    ...
    protected void addPredefinedRolesWithDefaultPermissions(com.netgrif.application.engine.objects.importer.model.Transition importTransition, Transition transition) {
    ...
    protected AssignPolicy toAssignPolicy(com.netgrif.application.engine.objects.importer.model.AssignPolicy policy) {
    ...
    protected DataFocusPolicy toDataFocusPolicy(com.netgrif.application.engine.objects.importer.model.DataFocusPolicy policy) {
        if (policy == null || policy.value() == null) {
    ...
    protected FinishPolicy toFinishPolicy(com.netgrif.application.engine.objects.importer.model.FinishPolicy policy) {
        if (policy == null || policy.value() == null) {

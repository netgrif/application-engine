...
public class ElasticPetriNetService implements IElasticPetriNetService {
    ...
    public List<PetriNet> findAllByUriNodeId(String uriNodeId) {
        List<com.netgrif.application.engine.adapter.spring.elastic.domain.ElasticPetriNet> elasticPetriNets = repository.findAllByUriNodeIdAndWorkspaceId(uriNodeId, userService.getLoggedOrSystem().getWorkspaceId());

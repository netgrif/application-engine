...
public class UriService implements IUriService {
    ...
    public UriNode save(UriNode uriNode) {
        return uriNodeRepository.save((com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode) uriNode);
    }
    ...
    public UriNode findById(String id) {
        Optional<com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode> navNodeOptional = uriNodeRepository.findById(id);
    ...
    public UriNode move(UriNode node, String destUri) {
        ...
        uriNodeRepository.saveAll(List.of((com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode) oldParent, (com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode) newParent, (com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode) node));

        List<com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode> childrenToSave = new ArrayList<>();
    ...
    private List<com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode> moveChildrenRecursive(String oldParentPath, String newParentPath, Set<UriNode> nodes) {
        List<com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode> updated = new ArrayList<>();
        ...
        for (UriNode node : nodes) {
            ...
            updated.add((com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode) node);
    ...
    public UriNode getOrCreate(String uri, UriContentType contentType) {
        ...
        for (int i = 0; i < pathLength; i++) {
            ...
            if (uriNode == null) {
                uriNode = new com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode();
            ...
            uriNode = uriNodeRepository.save((com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode) uriNode);
            if (parent != null) {
                parent.getChildrenId().add(uriNode.getStringId());
                uriNodeRepository.save((com.netgrif.application.engine.adapter.spring.petrinet.domain.UriNode) parent);
            }
            if (i > 0) {

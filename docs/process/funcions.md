...
Petriflow functions provide oportunity to declare your own functions without any changes in applications code. 
...
```
Only one function per element is allowed. Method overloading is also implemented and if more than one method with same method signature are present exception is throwed. Functions are compiled in run-time and like actions have access to ``ActionDelegate``. Also actions have access to one each other with one exception that global functions can call only other global function not process.
